    ...
    if (foundUser) {
        res.format({
            'text/plain': () => {
                res
                .status(HttpStatus.OK.code)
                .send(`id: ${foundUser.id}, firstName: ${foundUser.firstName}, lastName: ${foundUser.lastName}, email: ${foundUser.email}`);
            },
            'text/html': () => {
                res
                .status(HttpStatus.OK.code)
                .send(`<html><body><h1>User</h1><p>id: ${foundUser.id}</p><p>firstName: ${foundUser.firstName}</p><p>lastName: ${foundUser.lastName}</p><p>email: ${foundUser.email}</p></body></html>`);
            },
            'application/json': () => {
                res
                .status(HttpStatus.OK.code)
            ...
            default: () => {
                ...
                .send(new Response(HttpStatus.NOT_ACCEPTABLE.code, HttpStatus.NOT_ACCEPTABLE.status, `Nieobslugiwany typ danych`));
            }
        })
